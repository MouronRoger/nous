...
CURSOR_SETTINGS: Path = CURSOR_DIR / "settings.json"
...
def setup_framework() -> None:
    ...
    cursor_settings = {
        ...
        "ai": {
            "contextPrompts": [
                ...
                "",
                "This project uses the Akro document-driven development framework:",
                "- Documentation is the source of truth",
                "- Follow stage documents exactly as specified",
    ...
    if CURSOR_SETTINGS.exists():
        try:
            existing_settings = json.loads(CURSOR_SETTINGS.read_text())
            ...
            cursor_settings = existing_settings
        except json.JSONDecodeError:
            # If existing file is invalid, just use our new settings
            pass
    
    # Write settings
    CURSOR_SETTINGS.write_text(json.dumps(cursor_settings, indent=2))
    print(f"Configured {CURSOR_SETTINGS}")
...
def sync_memory() -> None:
    ...
    for category, count in categories.items():
        print(f"  - {count} {category} documents")
    
    memory_entries = create_memory_entries(valid_docs)
    
    # Report relationship stats
    entities = [e for e in memory_entries if e["type"] == "entity"]
    relations = [r for r in memory_entries if r["type"] == "relation"]
    
    print(f"Created {len(entities)} entities and {len(relations)} relationships")
